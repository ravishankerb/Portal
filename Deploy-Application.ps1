...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		## <Perform Installation tasks here>
			#Start Transcription
			#Start Transcription
			Start-Transcript -Path $MigrationPath\Logs\AD2AADJ.txt -Append -Force -NoClobber
			...
			function Test-ProvisioningPack {
				...
				Insert-MigrationStatus "Deploying Application" "Provisioning package checked for prior installation" "Deploy-Application.ps1" "Info"
			}
			...
			function Add-LocalUser {
				...
				Insert-MigrationStatus "Deploying Application" "Temporary user created" "Deploy-Application.ps1" "Info"
			}
			...
			function Set-Autologin {
				...
				Set-ItemProperty $RegPath "DefaultPassword" -Value $TempPassword -type String -Verbose

				Insert-MigrationStatus "Deploying Application" "Temporary user set for auto login" "Deploy-Application.ps1" "Info"
			}
			...
			function Disable-OOBEPrivacy  {
				...
				Insert-MigrationStatus "Deploying Application" "Disabled OOBE Privacy" "Deploy-Application.ps1" "Info"
			}
			...
			function Remove-Hybrid {
				...
				If($AzureADJoined -eq 'Yes'){
					...
					$dsregOutput = Invoke-Expression -Command "cmd /c C:\Windows\System32\dsregcmd.exe /leave"

				}

				Insert-MigrationStatus "Deploying Application" "Removed Entra Hybrid join" "Deploy-Application.ps1" "Info"
			}
			...
			function Remove-ADJoin {
				...
				If($PartOfDomain){
					...
					If(Test-Connection $domain){
						...
						$pw = $TempPassword | ConvertTo-SecureString -asPlainText -Force
						$usr = $TempUser
						$creds = New-Object System.Management.Automation.PSCredential($usr, $pw)
						$pc = "localhost"
